...
export async function bootstrapAdmin(accounts: AccountService, bootstrapFile: string): Promise<BootstrapResult> {
  ...
  if (!(await pathExists(bootstrapFile))) {
    await writeFile(`${bootstrapFile}.example`, JSON.stringify({ username: "admin", password: "change-me-now", display_name: "Server Admin" }, null, 2), "utf8");
  ...
  await accounts.createAccount({ username: input.username, password: input.password, displayName: input.display_name ?? input.username, role: "admin", steam64: "", mustChangePassword: false });
